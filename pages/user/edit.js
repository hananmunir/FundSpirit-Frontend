...
const PersonalComponent = ({ user }) => {
  ...
  const handleSubmit = (e) => {
    ...
    editUser(user.user.user._id, reqData, user.user.token)
      .then((res) => {
          ...
          <div className='d-flex flex-row align-items-center my-4 '>
            <img
              ...
              className={`rounded-circle ${
                window.innerWidth < 768 ? "mb-3" : "me-4"
              }`}
            />
...
const PasswordComponent = ({ user }) => {
  ...
  const validate = () => {
    return currentPassword.trim().length <= 0 || newPassword.trim().length <= 0;
  };
  const handleSubmit = (e) => {
    const reqData = {
    ...
    editPassword(user.user.user._id, reqData, user.user.token)
      .then((res) => {
        dispatch(logout());
        toast("User Password Updated", { type: "success" });
...
const Edit = () => {
  ...
  return (
    ...
    >
      <Row>
        <Col sm={12} md={8} lg={7} className='d-flex flex-column m-auto '>
          <h2>Edit Profile</h2>
          <span className='text-secondary'>
            Edit your Profile information below
          </span>
        </Col>
        <Col sm={12} className='pt-4 d-flex justify-content-center'>
          <div className='flex-row w-25 d-flex justify-content-between align-items-center '>
            <span
              className={`pointer fs-5 ${selection === 0 && "text-primary"}`}
              onClick={() => setSelection(0)}
            >
              Profile
            </span>
            <span
              className={`pointer fs-5   ${selection === 1 && "text-primary"}`}
        ...
        <Col sm={12}>
          {selection === 0 ? (
            <PersonalComponent user={state} />
          ) : (
            <PasswordComponent user={state} />
          )}
        </Col>
      </Row>
    </Container>
